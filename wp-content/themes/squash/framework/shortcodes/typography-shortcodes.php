...
function mo_segment_shortcode($atts, $content = null) {
    ...
    if (!empty($style) || !empty ($background_url) || !empty($background_color) || !empty($background_pattern)) {
        $inline_style = ' style="';
        if (!empty($background_image)) {
            $inline_style .= 'background:url(' . $background_image . ') no-repeat center center ' . $background_color . '; background-size: cover;';
            if ($fixed_background == 'true')
                $inline_style .= 'background-attachment:fixed;';
        }
        elseif (!empty($background_pattern)) {
            $inline_style .= 'background:url(' . $background_pattern . ') repeat left top ' . $background_color . ';';
        }
        ...
        $output = '<div ' . $id . ' class="segment clearfix ' . $class . '" ' . $inline_style . '>';
        $output .= '<div class="segment-content">' . do_shortcode($content) . '</div>';
        $output .= '</div><!-- .segment-->';
    }
    else {
        $output = '<div ' . $id . ' class="segment clearfix ' . $class . '"><div class="segment-content">' . do_shortcode($content) . '</div></div><!-- .segment-->';
    }
