...
function mo_custom_css_for_entry() {
    ...
    $background = mo_set_background($background);
    if (!empty($background)) {
        $output .= "\n" . '#custom-before-content-area-wrap  {';
        $output .= $background;
...
function mo_custom_css() {
    ...
    if (empty($heading_font)) {
        $heading_font = mo_get_theme_option('mo_heading_font', 'Open Sans');
    ...
    if ($skin_font_color) {
        ...
        $output .= "\n";
        $output .= 'a, a:active, a:visited, .fancy-tab li:hover a, .post-snippets .hentry .entry-title a:hover, ';
        $output .= '.sidebar a:hover, .read-more a, .top-of-page a:hover, #before-content-area a, #before-content-area a:active, #before-content-area a:visited';
        ...
        $output .= '#before-content-wrap.title-area, div.service-box1:hover, #portfolio-filter a:hover, ';
        $output .= '#slider-testimonials .flex-direction-nav li:hover';
        $output .= '{ background:' . $skin_font_color . '}';
        $output .= "\n";

        $output .= '.fancy-tab li:hover, .fancy-tab .current-tab, #before-content-wrap.title-area , ';
        $output .= '{ border-bottom-color: ' . $skin_font_color . '; }';
        $output .= "\n";

        $output .= '.rounded-button, #before-content-wrap.title-area ';
    ...
    if (!empty($post_tagline_font_size) || !empty($post_tagline_font_color)) {
        $output .= "\n" . '#before-content-area h2.tagline {';
    ...
    $bottom_area_widget_title_font_color = mo_get_theme_option('mo_bottom_area_widget_title_h3_font_color');
    if ($bottom_area_widget_title_font_color)
        $output .= "\n" . '#sidebars-footer .widget-title { color:' . $bottom_area_widget_title_font_color . ' !important; }';
    ...
    $entry_meta_border_color = mo_get_theme_option('mo_entry_meta_border_color');

    if (!empty($entry_meta_font_color) || !empty($entry_meta_font_size) || !empty($entry_meta_border_color)) {
        ...
        if (!empty($entry_meta_border_color)) {
            $output .= 'border-top: 1px solid ' . $entry_meta_border_color . ';';
            $output .= 'border-bottom: 1px solid ' . $entry_meta_border_color . ';';
        }
        $output .= '}';
    }
    ...
    $entry_meta_font_color = mo_get_theme_option('mo_bottom_area_entry_meta_font_color');
    if (!empty($entry_meta_font_color)) {
        $output .= "\n" . '#bottom-area ul.post-list .entry-meta, #bottom-area .post-list .byline { color:' . $entry_meta_font_color . ';}';
    }

    $bottom_area_text_font_color = mo_get_theme_option('mo_bottom_area_text_font_color');
    if (!empty($bottom_area_text_font_color))
        $output .= "\n" . '#bottom-area { color:' . $bottom_area_text_font_color . ' !important; }';

    $bottom_area_link_font_color = mo_get_theme_option('mo_bottom_area_link_font_color');
    if (!empty($bottom_area_link_font_color))
        $output .= "\n" . '#bottom-area a { color:' . $bottom_area_link_font_color . ' !important; }';

    $bottom_area_link_font_hover_color = mo_get_theme_option('mo_bottom_area_link_hover_font_color');
    if (!empty($bottom_area_link_font_hover_color))
        $output .= "\n" . '#bottom-area a:hover { color:' . $bottom_area_link_font_hover_color . ' !important; }';
    ...
    if ($logo_margin_top)
        $output .= "\n" . '#header-logo { top:' . $logo_margin_top . 'px; }';
    ...
    if ($nivo_slider_height) {
        ...
        $output .= "\n" . '#nivo-slider-wrapper { background: none; /* get rid of that existing shadow */ }';
        $output .= "\n" . '#nivo-slider {';
    ...
    $background = mo_get_theme_option('mo_bottom_area_background');
    $background = mo_set_background($background);
    if (!empty($background)) {
        $output .= "\n" . '#bottom-area {';
        $output .= $background;
    ...
    if (mo_browser_supports_css3_animations()) {
        ...
        if (empty($disable_smooth_page_load)) {
            $output .= "\n" . '#content, #before-content-area, #custom-before-content-area, ' . "\n";
            $output .= "\n" . '.first-segment .heading2, .first-segment .heading1, sidebar-right-nav, sidebar-left-nav , ' . "\n";
