...
class MO_LayoutManager {
    ...
    function initialize() {
        ...
        /* Manage Post Layouts. */
        add_action('admin_menu', array(&$this, 'post_layouts_admin_setup'));
    ...
    function post_layouts_admin_setup() {
        ...
        add_meta_box('post-layouts-post-meta-box', __('Post Layout', 'mo_theme'), array(&$this, 'add_post_layouts_meta_box'), 'post', 'side', 'default');

        add_action('save_post', array(&$this, 'post_layouts_save_post'), 10, 2);
    }
    ...
    function manage_theme_styling($classes) {
        ...
        if (isset($_COOKIE['mo_boxed_layout']) || (mo_get_theme_option('mo_theme_layout') == 'Boxed'))
            $classes[] = 'boxed';
    ...
    function get_current_layout() {
        ...
        } elseif (is_attachment() || is_singular('portfolio')) {
    ...
    function get_post_snippets_layout($args) {
        ...
        elseif (empty($taxonamy) || empty($terms))
            $loop = new WP_Query(array('post_type' => $post_type, 'posts_per_page' => $post_count)); else
        ...
        if ($loop->have_posts()) :
            ...
            $output .= '<ul class="image-grid ' . $layout_class . '">';
            ...
            while ($loop->have_posts()) : $loop->the_post();
                ...
                if (!$hide_thumbnail) {
                    if ($post_type == 'post')
                        ...
                        $thumbnail_output = mo_get_thumbnail(array('image_size' => $thumbnail_size, 'taxonamy' => $taxonamy));
    ...
    function get_post_snippets_list($args) {
        ...
        if ($loop->have_posts()) :
            ...
            while ($loop->have_posts()) : $loop->the_post();
                ...
                if (!$hide_thumbnail) {
                    $thumbnail_url = mo_get_thumbnail(array('show_image_info' => true, 'image_size' => $thumbnail_size));
