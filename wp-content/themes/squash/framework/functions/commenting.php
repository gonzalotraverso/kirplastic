...
function mo_comment_published() {
    $link = '<span class="published">' . sprintf(__('%1$s at %2$s', 'mo_theme'), '<abbr class="comment-date" title="' . get_comment_date(esc_attr__('l, F jS, Y, g:i a', 'mo_theme')) . '">' . get_comment_date() . '</abbr>', '<abbr class="comment-time" title="' . get_comment_date(esc_attr__('l, F jS, Y, g:i a', 'mo_theme')) . '">' . get_comment_time() . '</abbr>');
    return $link;
}
