...
function mo_image_shortcode($atts, $content = null, $code) {
    ...
    if (!isset($height) && !isset($width)) {
        $image_size = mo_get_wp_image_size($size, 'medium'); // default to medium if size is invalid
    ...
    if (mo_to_boolean($image_frame)) {
        ...
        $wrap .= '<div class="image-container  ' . $size . '">';
        $wrap .= '<div class="image-frame"><div class="image-area preloader">';
        $wrap .= $output;
        $wrap .= '</div></div>';
        $wrap .= '</div></div>';
