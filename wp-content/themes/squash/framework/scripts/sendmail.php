...
if ($post) {
    ...
    if (empty($contact_url))
        $subject .= __(' Contacto - Kirplastic', 'mo_theme');
    else
    ...
    if (!empty($contact_reason))
        $message .= "\n" . __('Nombre: ', 'mo_theme') . $name;
    $phone_number = trim($_POST['contact_phone']);
    if (!empty($phone_number))
        $message .= "\n\n" . __('Numero de telefono: ', 'mo_theme') . $phone_number;
    if (!empty ($email))
        $message .= "\n\n" . __('Email: ', 'mo_theme') . $email;
    if (!empty($contact_url))
    ...
    if (!empty($company)) {
        $message .= "\n\n".__('Compañia: ', 'mo_theme') . $company;
    }

    $message .= "\n\n" . __('Mensaje: ', 'mo_theme') . stripslashes($_POST['message']);
