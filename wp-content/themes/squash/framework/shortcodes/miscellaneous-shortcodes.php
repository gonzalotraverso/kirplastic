...
/* Shortcode for placing Adsense ad anywhere in the post - input left or right for alignment */

function mo_adsense_shortcode($atts, $code) {

    extract(shortcode_atts(array('align' => false), $atts));

    $adsense_ad = mo_get_theme_option('mo_adsense_code', '');

    return '<div class="adsense-block' . ($align ? ' align' . $align : '') . '">' . $adsense_ad . '</div>';
}

add_shortcode('adsense', 'mo_adsense_shortcode');
